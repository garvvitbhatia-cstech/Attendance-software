...
class ProfileController extends Controller {
    ...
    public function saveProfile(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            self::$UserModel->where(array('id' => $request->session()->get('admin_id')))->update(array('profile' => $actual_image_name, 'name' => trim($request->input('name'))));
            return json_encode(array('heading' => 'Success', 'msg' => 'Profile updated successfully'));
            die;
        }
    }
    ...
    public function updatePassword(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if ($errors->first('current_password')) {
                return json_encode(array('heading' => 'Error', 'msg' => $errors->first('current_password')));
                die;
                ...
                return json_encode(array('heading' => 'Error', 'msg' => $errors->first('confirm_password')));
                die;
            }
        } else {
            ...
            if ($User) {
                ...
                if (!$PasswordMatch) {
                    ...
                    return json_encode(array('heading' => 'Success', 'msg' => 'Password updated successfully'));
                    die;
                }
            } else {
    ...
    public function saveSetting(Request $request) {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'halfday_time' => 'required', 
            'absent_time' => 'required', 
            'holiyday_allowed' => 'required'
        ], [
            'admin_email.required' => 'Please enter your email.', 
            'admin_email.email' => 'Please enter valid email.', 
            'halfday_time.required' => 'Please enter halfday time.', 
            'absent_time.required' => 'Please enter absent time.', 
            'holiyday_allowed.required' => 'Please enter number of holiday.'
        ]);
        if ($validator->fails()) {
            $errors = $validator->errors();
            if ($errors->first('admin_email')) {
                return json_encode(array('heading' => 'Error', 'msg' => $errors->first('admin_email')));
                die;
            }
            if ($errors->first('holiyday_allowed')) {
                return json_encode(array('heading' => 'Error', 'msg' => $errors->first('holiyday_allowed')));
                die;
            }
            if ($errors->first('halfday_time')) {
                return json_encode(array('heading' => 'Error', 'msg' => $errors->first('halfday_time')));
                die;
            }
            if ($errors->first('absent_time')) {
                return json_encode(array('heading' => 'Error', 'msg' => $errors->first('absent_time')));
                die;
            }
        } else {
            # profile pic upload
            if (isset($request->logo) && $request->logo->extension() != "") {
                $validator = Validator::make($request->all(), ['logo' => 'required|image|mimes:jpeg,png,jpg,png,svg|max:20480']);
                if ($validator->fails()) {
                    $errors = $validator->errors();
            ...
            self::$Settings->where(array('id' => 1))->update(
                array(
                    ...
                    'halfday_time' => trim($request->input('halfday_time')), 
                    'absent_time' => trim($request->input('absent_time')), 
                    'holiyday_allowed' => trim($request->input('holiyday_allowed')
            )));
            return json_encode(array('heading' => 'Success', 'msg' => 'Profile updated successfully'));
            die;
        }
