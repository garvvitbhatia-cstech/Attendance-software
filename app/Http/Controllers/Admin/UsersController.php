...
class UsersController extends Controller {
    ...
    public function getList(Request $request) {
        ...
        if ($request->session()->get('admin_type') == 'Account') {
            return redirect('/admin/');
        }
            ...
            if ($validator->fails()) {
                ...
                if (!self::$User->ExistingRecord($request->input('email'))) {
                    ...
                    if (isset($request->profile_image) && $request->profile_image->extension() != "") {
                        $validator = Validator::make($request->all(), ['profile_image' => 'required|image|mimes:jpeg,png,jpg|max:2048']);
                        if ($validator->fails()) {
                            $errors = $validator->errors();
                            return json_encode(array('heading' => 'Error', 'msg' => $errors->first('profile_image')));
                            die;
                        } else {
                ...
                echo json_encode(array('heading' => 'Success', 'msg' => 'Customer added successfully'));
                die;
            }
        }
        ...
        if ($request->input()) {
            ...
            if ($validator->fails()) {
                ...
                if (self::$User->ExistingRecordUpdate($request->input('email'), $RowID)) {
                    echo json_encode(array('heading' => 'Error', 'msg' => 'Customer already exists.'));
                    die;
                } else {
                    $setData['id'] = $RowID;
                    $setData['name'] = $request->input('name');
                ...
                echo json_encode(array('heading' => 'Success', 'msg' => 'Customer updated successfully'));
                die;
            }
        }
